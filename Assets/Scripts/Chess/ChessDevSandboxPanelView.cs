...
public class ChessDevSandboxPanelView : MonoBehaviour
{
    ...
    static Button AddButton(Transform parent,string label,out TMP_Text text){var r=CreateRect(label.Replace(" ","")+"Button",parent);r.sizeDelta=new Vector2(0,36);var img=r.gameObject.AddComponent<Image>();img.color=ChessUITheme.ButtonNormal;var b=r.gameObject.AddComponent<Button>();text=CreateText("Label",r,label);text.alignment=TextAlignmentOptions.Center;var tr=text.rectTransform;tr.anchorMin=Vector2.zero;tr.anchorMax=Vector2.one;tr.offsetMin=tr.offsetMax=Vector2.zero;return b;}
    static Toggle AddToggle(Transform parent,string label){var tGo=CreateRect("AIToggle",parent);var tg=tGo.gameObject.AddComponent<Toggle>();var bg=CreateRect("Background",tGo);bg.sizeDelta=new Vector2(20,20);bg.gameObject.AddComponent<Image>().color=ChessUITheme.PanelSecondary;var check=CreateRect("Checkmark",bg);check.sizeDelta=new Vector2(14,14);var checkImage=check.gameObject.AddComponent<Image>();checkImage.color=ChessUITheme.GoldAccent;tg.graphic=checkImage;tg.targetGraphic=bg.GetComponent<Image>();var txt=CreateText("Label",tGo,label);txt.fontSize=20;txt.rectTransform.anchoredPosition=new Vector2(70,0);return tg;}
    static TMP_InputField AddInput(Transform parent){var r=CreateRect("FenInput",parent);r.sizeDelta=new Vector2(0,100);var img=r.gameObject.AddComponent<Image>();img.color=ChessUITheme.PanelSecondary;var text=CreateText("Text",r,"");text.fontSize=18;text.color=ChessUITheme.GoldAccent;text.rectTransform.offsetMin=new Vector2(8,8);text.rectTransform.offsetMax=new Vector2(-8,-8);var placeholder=CreateText("Placeholder",r,"Enter FEN");placeholder.fontSize=18;placeholder.color=ChessUITheme.MutedText;placeholder.rectTransform.offsetMin=new Vector2(8,8);placeholder.rectTransform.offsetMax=new Vector2(-8,-8);var input=r.gameObject.AddComponent<TMP_InputField>();input.textViewport=r;input.textComponent=(TextMeshProUGUI)text;input.placeholder=placeholder;input.lineType=TMP_InputField.LineType.MultiLineNewline;return input;}
}
